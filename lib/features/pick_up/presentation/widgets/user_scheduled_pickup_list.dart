        ...
        >(
          ...
          builder: (context, pickupState) {
            if (pickupState.scheduledPickup == null ||
                pickupState.scheduledPickup!.isEmpty) {
              return const PickupsEmptyState();
            } else {
              return Column(
                children: [
                  SchedulePickUpTab(
                    upcomingSchedulesPickupLength: pickupState.scheduledPickup?.length ?? 0,
                    historySchedulesPickUpLenght: 0,
                  ),
                  Expanded(
                    child: Skeletonizer(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          return SchedulePickCard(
                            ...
                            handleCancelPickUp: () {
                              FloAidDialog.show(
                                ...
                                onConfirm: () {
                                  ...
                                  final uId =
                                      context.read<AuthBloc>().state.user?.uuid;
                                  if (uId != null) {
                                    context
                                        .read<GetUsersScheduledPickupsBloc>()
                                        .add(
                                          FetchUserScheduledPickUpsEvent(
                                            userId: uId,
                                          ),
                                        );
                                  }
                            ...
                            handleEditPickUp: () {
                              final pickupToEdit =
                                  pickupState.scheduledPickup?[index];
